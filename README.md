...
目前資料格式為 schema v2；應用程式會在讀取時自動遷移 v1 的本機資料與 JSON 備份，並保留原本的像素字級外觀。
...
畫布使用 dynamic viewport、safe-area 與 VisualViewport 資訊；工具列位置以相對座標保存在瀏覽器，旋轉裝置、Safari 工具列伸縮或軟鍵盤改變可用空間時會重新夾限在可視範圍。網站不鎖定方向，也不全域攔截觸控捲動。
