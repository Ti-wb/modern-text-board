...
/**
 * Keeps motion below twelve CSS pixels per displayed frame, then opportunistically
 * snaps the step to a sixteenth physical pixel. The finer grid preserves the
 * 600+ px/s top range even on a 60Hz display. A snap is rejected when it would
...
export function resolveAdaptiveMarqueeSpeed(
  ...
  const alignedDevicePixelsPerFrame =
    Math.floor(unsnappedDevicePixelsPerFrame * 16 + 1e-7) / 16;
